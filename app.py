...
def _parse_month():
    """Get selected month from query param or session, default to current."""
...
def _get_branch_id():
    """Get branch_id from query param or session, default 126."""
...
def api_summary():
    ...
    fixed = fixed_row['total']

    # Salary
    salary_row = db.execute(
        "SELECT COALESCE(SUM(hours * rate), 0) as total FROM employee_hours "
        "WHERE branch_id = ? AND month = ?",
        (branch_id, month)
    ).fetchone()
    salary = salary_row['total']
