...
def _aggregate_summary_parts(parts, month, n):
    ...
    if lives:
        ...
        all_stale = all(l.get('is_stale') for l in lives)
        stale_dates = [l.get('stale_date') for l in lives if l.get('stale_date')]
        live = {
        ...
        if all_stale and stale_dates:
            live['stale_date'] = max(stale_dates)
    agg.update({
